...
[https://github.com/JoshWynnLim/Lim_JoshWynn_201_A06.git]
...
![](/Users/joshwynngokongweilim/Desktop/Gitupload.png)
...
for(name in names(player_info)){
  ppg <- (player_info[[name]]$points) / (player_info[[name]]$games_played)
  
  points_per_game[[name]] <- ppg
}
...
```{r}
lapply(points_per_game, round, 2)
```
...
rounded_matrix <- apply(mat_rand, MARGIN = 1:2, roundin)
...
for(i in 3:ncol(max_temps)){
  temp <- (((mean(max_temps[[i]], na.rm = TRUE))))
  values[[i-2]] <- list(month = months[i-2], temp = temp)
}
print(values)
...
```{r}
Colorado <- max_temps[1:126, c(1:14)]
Ohio <- max_temps[127:252, c(1:14)]
Virginia <- max_temps[253:378, c(1:14)]
```
...
```{r}
medianC <- apply(Colorado[3:14], 1, median, na.rm = TRUE)
max_med <- max(medianC)
Col_max <- Colorado$year[which.max(medianC)]
min_med <- min(medianC)
Col_min <- Colorado$year[which.min(medianC)]

print(paste0("The maximum median annual temperature of ", max_med, " occured in ", Col_max, " and the minimum median annual temeperature of ", min_med, " occured in ", Col_min, "."))
```
